...
class Trainer:
  ...
  def trainOnData(self, original, epochs, epochsBeforeUpdating, use_target = True, save=True):
    ...
    for b in range(epochs // epochsBeforeUpdating):
      ...
      for i in range(epochsBeforeUpdating):
        l = np.sum(self.gym.agent.train(epochs = 1, batch_size=32, notGonnaBeData=False, data = self.improveQualityOfData(data), verbose = 0, use_target=use_target))
        print(f"\tTIME: {round((time.time() - s_time) * 10) / 10}\tLOSS {(b * epochsBeforeUpdating + i + 1)} / {epochs}: {l}")
      # except:
      #   print("ERROR")
...
if __name__ == "__main__":
  ...
  import time
  s_t = time.time()
  trainer.trainOnData(all_data[:1000],1, 1, False)
  print(time.time() - s_t)
  s_t = time.time()
  trainer.trainOnData(all_data[:1000],1, 1, True)
  print(time.time() - s_t)
